"""
An hdf5 file based sampler for selene.  MatFileSample was used as a
...
def _load_hdf5_file(filepath, sequence_key, targets_key=None):
    """
...
class Hdf5FileSampler(FileSampler):
    """
    A sampler for which the dataset is loaded directly from a `*.h5` file.
    ...
    def __init__(self,
        ...
        """
        Constructs a new `Hdf5FileSampler` object.
        """
        super(Hdf5FileSampler, self).__init__()
        out = _load_hdf5_file(
            filepath,
            sequence_key,
